...
func New(ctx context.Context, cfg Config) (*Client, error) {
	client, err := NewUnverified(cfg)
	if err != nil {
		return nil, err
	}
	pingCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	if _, err := client.c.ListBuckets(pingCtx); err != nil {
		return nil, fmt.Errorf("ping minio: %w", err)
	}
	return client, nil
}

func NewUnverified(cfg Config) (*Client, error) {
	if cfg.Endpoint == "" {
...
func (c *Client) PutObject(ctx context.Context, bucket, key, contentType string, data []byte) (int64, error) {
	info, err := c.c.PutObject(ctx, bucket, key, bytes.NewReader(data), int64(len(data)), minio.PutObjectOptions{
